...
describe('AppController', () => {
  ...
  it('should send news as an article carousel and sub-category buttons if category is selected', async () => {
    ...
    jest
      .spyOn(userService, 'saveButtonResponse')
    ...
    expect(userService.saveButtonResponse).toHaveBeenCalledWith(
      ...
      it('should send news as an article carousel and go back to main menu if subcategory is selected', async () => {
        ...
        jest.spyOn(messageService, 'getCategoryID').mockResolvedValueOnce('49');
        jest.spyOn(khabriMediaService, 'sendNewsAsArticleCarousel').mockResolvedValueOnce(2);
        jest.spyOn(messageService, 'goBackToMainMenu').mockResolvedValueOnce({ id: '83d9db82-b3b0-4802-b2d8-a92f481435c6' });
        ...
        expect(messageService.getCategoryID).toHaveBeenCalledWith('Sports1', 'English');
        expect(khabriMediaService.sendNewsAsArticleCarousel).toHaveBeenCalledWith('English', process.env.BOT_ID, '1234567890', '49', 'Sports1', 1, 2);
        expect(messageService.goBackToMainMenu).toHaveBeenCalledWith('1234567890', 'English');
      });
    ...
    it('should send category buttons when given valid \'button_response\' with value \'Go Back To Main Menu\'', async () => {
      ...
      // Assert
      expect(result).toBe('ok');
      expect(messageService.categoryButtons).toHaveBeenCalledWith(body.from, 'English');
    });
        ...
        it('should send sub-category buttons when given valid \'button_response\' with value \'Browse Other Categories\'', async () => {
          ...
          // Assert
          expect(messageService.sub_categoryButtons).toHaveBeenCalledWith('1234567890', 'English', 'Sports');
